...
Usage: 
    plot_observable_TTV.py [image_list] [confirm_TTV]
Editor:
    Jacob975
20181219
#################################
...
if __name__ == "__main__":
    ...
    if len(argv) != 3:
        ...
        print "Usage: plot_observable_TTV.py [images name] [confirm_TTV]"
        exit(1)
    image_name = argv[1]
    transit_table_name = argv[2]
    #----------------------------------------
    # Load confirm TTV
    index_Vmag = 11
    index_depth = 9
    ...
    Vmag = transit_table[:,index_Vmag]
    depth = transit_table[:,index_depth]
    plt.scatter(np.array(Vmag, dtype = float), 
                np.divide(np.array(depth, dtype = float), 100), 
                s = 2, 
                label = 'Confirm transits')
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig('{0}_observable_ttv.png'.format(image_name[:-5]))
    #---------------------------------------
    # Measure time
