...
'''
Program:
This is a program to match a list of fits, and save a log about shift 
...
update log
    ...
    20170918 version alpha 6
    1.  a branch of fit_move.py
    2.  The code works.
'''
...
ecc = 1
ref_star_list = []
while len(ref_star_list) > 50 or len(ref_star_list) < 3:
    hwl += 1
...
name_shift_log = "shift_log"
temp = "rm {0}".format(name_shift_log)
os.system(temp)
shift_log = open( name_shift_log, "a")
...
for order in xrange(len(fits_list)):
    ...
    shift_log.write("{0}, {1:.4f}+-{2:.4f}, {3:.4f}+-{4:.4f}\n".format(fits_list[order], mean_delta_x, std_delta_x, mean_delta_y, std_delta_y))
